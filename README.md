...
- `.github/workflows/deploy.yml` — `deploy-legacy` (release events only) + `deploy-swarm` (release **and** workflow_dispatch). Both jobs now wait up to 5 minutes for Django to answer HTTP before being marked successful; swarm runs remain non-blocking on release events during cutover, while `workflow_dispatch` runs fail loudly.
