...
from toxtempass.evaluation.post_processing.utils import generate_comparison_csv, has_answer_not_found
...
def select_question_set(label: str | None = None) -> QuestionSet:
    ...
    qs = (
        QuestionSet.objects.filter(is_visible=True)
        .order_by("-created_at")
        .first()
    )
    if not qs:
...
def run(
    ...
    processed_scored_dir = processed_scored_dir or eval_config.tier1_processed_scored
    raw_dir = raw_dir or eval_config.tier1_raw
    output_base_dir = output_base_dir or eval_config.tier1_output
    ...
    for model_config in models:
        ...
        for json_file, pdf_file_path in tqdm(
            ...
            assay = AssayFactory(title=pdf_file, description="", question_set=question_set)
            questions = (
                Question.objects.filter(subsection__section__question_set=question_set)
