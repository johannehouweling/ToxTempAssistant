...
BASE_DIR = Path("myocyte/toxtempass/evaluation/positive_control/output/o3-mini")
OUTPUT_CSV = Path("myocyte/toxtempass/evaluation/data_analysis_plotting/christophe_data_analysis/results/tables/answer_frequency_positive_controls/answer_freq_o3-mini.csv")
...
for file in csv_files:
    df_pco3mini = pd.read_csv(file)
    df_pco3mini["source_file"] = file.name
    df.append(df_pco3mini)

combined_o3mini_df = pd.concat(df)

print(combined_o3mini_df.head())
print(combined_o3mini_df.shape)
print(combined_o3mini_df.columns)

# add a column giving a value of 1 to answered questions and 0 to unanswered questions by the LLM
combined_o3mini_df["answer_given"] = (
    ~combined_o3mini_df['llm_answer']
    .str.contains(NOT_FOUND_STRING, regex=False)
    ).astype(int)

print(combined_o3mini_df['answer_given'])

# removing the rows which have no ground truth
with_gtruth_o3mini = combined_o3mini_df.dropna(subset=['gtruth_answer'])

print(with_gtruth_o3mini.shape)

# create an average of answered/unanswered over the documents per question
summary = with_gtruth_o3mini.groupby("question", as_index=False)["answer_given"].mean()
...
plt.title ("gpt-o3-mini question answer frequency")
