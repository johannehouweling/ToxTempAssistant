...
BASE_DIR = Path("myocyte/toxtempass/evaluation/positive_control/output/gpt-4o-mini")
OUTPUT_CSV = Path("myocyte/toxtempass/evaluation/data_analysis_plotting/christophe_data_analysis/results/tables/answer_frequency_positive_controls/answer_freq_4o-mini.csv")
...
for file in csv_files:
    df_pc4omini = pd.read_csv(file)
    df_pc4omini["source_file"] = file.name
    df.append(df_pc4omini)

combined_4omini_df = pd.concat(df)

print(combined_4omini_df.head())
print(combined_4omini_df.shape)
print(combined_4omini_df.columns)

# add a column giving a value of 1 to answered questions and 0 to unanswered questions by the LLM
combined_4omini_df["answer_given"] = (
    ~combined_4omini_df['llm_answer']
    .str.contains(NOT_FOUND_STRING, regex=False)
    ).astype(int)

print(combined_4omini_df['answer_given'])

# removing the rows which have no ground truth
with_gtruth_4omini = combined_4omini_df.dropna(subset=['gtruth_answer'])

print(with_gtruth_4omini.shape)

# create an average of answered/unanswered over the documents per question
summary = with_gtruth_4omini.groupby("question", as_index=False)["answer_given"].mean()
...
plt.title ("gpt-4o-mini question answer frequency")
