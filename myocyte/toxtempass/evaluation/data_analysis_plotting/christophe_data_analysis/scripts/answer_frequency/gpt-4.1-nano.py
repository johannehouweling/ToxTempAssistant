...
BASE_DIR = Path("myocyte/toxtempass/evaluation/positive_control/output/gpt-4.1-nano")
OUTPUT_CSV = Path("myocyte/toxtempass/evaluation/data_analysis_plotting/christophe_data_analysis/results/tables/answer_frequency_positive_controls/answer_freq_4.1-nano.csv")
...
for file in csv_files:
    df_pc41nano = pd.read_csv(file)
    df_pc41nano["source_file"] = file.name
    df.append(df_pc41nano)

combined_41nano_df = pd.concat(df)

print(combined_41nano_df.head())
print(combined_41nano_df.shape)
print(combined_41nano_df.columns)

# add a column giving a value of 1 to answered questions and 0 to unanswered questions by the LLM
combined_41nano_df["answer_given"] = (
    ~combined_41nano_df['llm_answer']
    .str.contains(NOT_FOUND_STRING, regex=False)
    ).astype(int)

print(combined_41nano_df['answer_given'])

# removing the rows which have no ground truth
with_gtruth_41nano = combined_41nano_df.dropna(subset=['gtruth_answer'])

print(with_gtruth_41nano.shape)

# create an average of answered/unanswered over the documents per question
summary = with_gtruth_41nano.groupby("question", as_index=False)["answer_given"].mean()
...
plt.title ("gpt-4.1-nano question answer frequency")
